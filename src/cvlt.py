...
def insert_record(job, hiring_organization, job_location, url):
    try:
        ...
        # print city_id, "       ", job_location
        cursor.execute(
            "INSERT INTO vacancy (site, city_id, time, date, jobtitle, hiring_organisation, job_location, url)"
            " VALUES (%s, %s,%s, %s, %s, %s, %s, %s)",
            (
                host, city_id, datetime.datetime.now(), datetime.datetime.now(), job, hiring_organization,
                job_location,
                url))
    except ValueError:
        print(ValueError)
...
def get_city_id(job_location):
    try:
        cursor.execute("SELECT id from city where name = %(name)s", {"name": job_location})
        ...
        cursor.execute(
            "INSERT INTO city (name) VALUES ('%s') RETURNING id;" % (job_location))
